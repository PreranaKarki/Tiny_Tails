...
import { Statuses, getHamster } from "../../Store/Slices/ProductSlice";
...
const CategoryHamster = () => {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const { categoryHamsters, status } = useSelector((state) => state.products);
  ...
  return (
    <>
      <Wrapper>
        {categoryHamsters.map((currentElement) => (
          <NavLink
...
export default CategoryHamster;
